...
function imagename_savedatabase($codpac,$filename){
	...
	$qryIM=sprintf('INSERT INTO db_media (file, estado) VALUES (%s,%s)',
	SSQL($filename,'text'),
	SSQL('1','int'));
	mysql_query($qryIM)or die(mysql_error());
	$idm=mysql_insert_id();
	
	$qryIP=sprintf('INSERT INTO db_pacientes_media (cod_pac, id_med) VALUES (%s,%s)',
	SSQL($codpac,'int'),
	SSQL($idm,'int'));
	mysql_query($qryIP)or die(mysql_error());
}
/* 
Description: 
