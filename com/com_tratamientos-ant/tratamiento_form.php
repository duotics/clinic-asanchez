...
$acc=vParam('action', isset($_GET['action']) ? $_GET['action'] : NULL, isset($_POST['action']) ? $_POST['action'] : NULL);
//Eliminar Tratamiento
if($acc=='DELTF'){ header(sprintf("Location: %s", '_fncts.php?idt='.$idt.'&action=DELTF')); }
//FORM
...
if($detTrat){
	...
	if($detTD){//Detalle Tratamiento
		$accTD='UPDD';
		$btnAccTD='<button class="btn btn-success btn-block" type="submit"><i class="fas fa-save fa-lg"></i> Actualizar Medicamento</button>';
	}else{
		$accTD='INSD';
		$btnAccTD='<button class="btn btn-primary btn-block" type="submit"><i class="fas fa-save fa-lg"></i> Agregar Medicamento</button>';
	}

}else{
	$acc='INS';
	$btntrat='<button type="submit" class="btn btn-large btn-info"><i class="fas fa-save fa-lg"></i> GUARDAR</button>';
	$detCon=detRow('db_consultas','con_num',$idc);
...
<fieldset>
    ...
    <input name="action" type="hidden" id="action" value="<? echo $acc?>">
...
<div class="row">
    ...
    <h4><i class="fas fa-plus-square fa-lg"></i> Medicamentos Disponibles</h4>
