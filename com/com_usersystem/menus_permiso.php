...
$detUsu=detRow('db_user_system','user_cod',$id);
if($detUsu){
	$acc='UPD';
...
<?php if($detUsu){ ?>
<?php
$qryMC=sprintf('SELECT * FROM db_menus');
$RSmc=mysql_query($qryMC);
$dRSmc=mysql_fetch_assoc($RSmc);
$tRSmc=mysql_num_rows($RSmc);
?>
...
<?php }while($dRSmc=mysql_fetch_assoc($RSmc)); ?>
</ul>
...
<?php foreach($LMC as $val){ ?>
	...
	$qryMP=sprintf('SELECT * FROM db_menus_items WHERE men_idc=%s AND men_padre=%s AND men_stat=%s',
	SSQL($val['id'],'int'),
	SSQL(0,'int'),
	SSQL(1,'int'));
	$RSmp=mysql_query($qryMP);
        ...
        <?php $estCheckMp=verifyCheckUserMenu($dRSmp['men_id'],$id) ?>
        	<div class="col-sm-2">
            	<div class="well well-sm well-white">
					...
					<?php //MENUS PADRES DEL MENU CONTENEDOR SELECCIONADO
                        $qryMI=sprintf('SELECT * FROM db_menus_items WHERE men_padre=%s AND men_stat=%s',
                        SSQL($dRSmp['men_id'],'int'),
                        SSQL(1,'int'));
                        $RSmi=mysql_query($qryMI);
                        ...
                        <?php if($tRSmi>0){ ?>
							...
							<?php }while($dRSmi=mysql_fetch_assoc($RSmi)); ?>
                        <?php } ?>

                </div>
            </div>
        <?php }while($dRSmp=mysql_fetch_assoc($RSmp)); ?>
        </div>
        </fieldset>
	<?php }else{ ?>
