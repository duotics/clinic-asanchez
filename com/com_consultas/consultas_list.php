...
$RScl = mysql_query($query_RScl) or die(mysql_error());
$row_RScl = mysql_fetch_assoc($RScl);
$totalRows_RScl = mysql_num_rows($RScl);
mysql_free_result($RScl);
...
if ($totalRows_RScl>0){
	...
	$RScl = mysql_query($query_RScl.' '.$pages->limit) or die(mysql_error());
	$row_RScl = mysql_fetch_assoc($RScl);
	$totalRows_RScl = mysql_num_rows($RScl);
}
...
<tbody>
	...
	?>
	  ...
	  <?php } while ($row_RScl = mysql_fetch_assoc($RScl)); ?>
</tbody>
</table>
