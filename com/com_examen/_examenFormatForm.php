...
if($dEFD){
	$accD=md5('UPDefd');
	$btnAccD='<button type="submit" class="btn btn-success"><i class="fas fa-save fa-lg"></i> ACTUALIZAR</button>';
}else{
	$accD=md5('INSefd');
...
<div>
  ...
  <div class="tab-content panel panel-default panel-body">
    ...
    <div role="tabpanel" class="tab-pane <?php echo $tabS[tabB] ?>" id="tabB">
        ...
        <?php
		$qLEFD=sprintf('SELECT * FROM db_examenes_format_det WHERE idef=%s',
					  SSQL($id,'int'));
		$RSlefd=mysql_query($qLEFD);
		$dRSlefd=mysql_fetch_assoc($RSlefd);
		$tRSlefd=mysql_num_rows($RSlefd);
		?>
    	<div class="row">
    		<div class="col-sm-5">
    			<fieldset class="form-horizontal well">
    				...
    				<div class="form-group">
    					<div class="col-sm-8 col-md-offset-4">
    						<?php echo $btnAccD ?>
							<a href="<?php echo $urlc ?>?id=<?php echo $id ?>" class="btn btn-default"><i class="fas fa-plus-square fa-lg"></i>NUEVO</a>
    					</div>
    				</div>
    			</fieldset>
    		</div>
    		<div class="col-sm-7">
    			...
    			<table class="table table-bordered">
    				...
    				<?php }while($dRSlefd=mysql_fetch_assoc($RSlefd)); ?>
    				</tbody>
    			</table>
    			<?php }else{ ?>
    			<div class="alert alert-info"><h4>No hay tipos de examen relacionados</h4></div>
    			<?php } ?>
    		</div>
    	</div>
		</form>
