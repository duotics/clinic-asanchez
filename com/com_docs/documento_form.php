...
if($action=='DELDF') header(sprintf("Location: %s", '_acc.php?idd='.$idd.'&action=DELDF'));
$detpac=dataPac($idp);
...
if($detdoc){
	...
	$btnform='<button type="submit" class="btn btn-success"><i class="fas fa-save fa-lg"></i> ACTUALIZAR</button>';
	$btnPrint='<a class="btn btn-info" href="'.$RAIZc.'com_docs/doc_print.php?id='.$idd.'"><i class="fas fa-print fa-lg"></i> IMPRIMIR</a>';
}else{
	$action='INS';
	$doc_date=date('Y-m-d');
	$doc_nom=$detdocf['nombre'];
	$doc_con=$detdocf['formato'];
	$btnform='<button type="submit" class="btn btn-primary"><i class="fas fa-save fa-lg"></i> GUARDAR</button>';
}
...
<div class="container-fluid">
    ...
    <div class="navbar-header">
      ...
      <a class="navbar-brand" href="#"><i class="fas fa-file fa-lg"></i> DOCUMENTO</a>
    </div>
...
<div class="row">
  ...
  <ul class="dropdown-menu">
		<?php do{?>
        <li><a href="<?php echo $RAIZc ?>com_docs/documento_form.php?idd=<?php echo $idd ?>&iddf=<?php echo $row_RSdf['id_df'] ?>&idp=<?php echo $idp ?>&idc=<?php echo $idc?>&action=NEW"><i class="icon-file"></i> <small>Cambiar a</small> <?php echo $row_RSdf['nombre'] ?></a></li>
