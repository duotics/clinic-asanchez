...
<?php
$vP=TRUE;//VERIFICACION PARA COMMIT O ROLLBACK
$vR=FALSE;//TRUE PARA RESETAR TODAS LAS TABLAS
mysql_query("SET AUTOCOMMIT=0;");
mysql_query("BEGIN;");
//////PROCESS DATA
$qry=sprintf('SELECT pac_cod FROM db_pacientes WHERE 1=1 LIMIT 0');
$RS=mysql_query($qry);
...
<?php
			...
			foreach($DIAGNOS as $key => $val){
				if(($key)&&($val)){
					...
					if(@mysql_query($qryD)){
						...
						$vP=FALSE;
						echo '<p>Error al crear diagnostico</p>'.mysql_error();
						break;
					}
				}
			}
	...
	<?php } ?>
		<td><?php echo $ID ?></td>
		<td>Visitas Encontradas. <?php echo $tRSb ?></td>
		<td>Visitas Creadas <?PHP echo $contCR ?></td>
	</tr>
