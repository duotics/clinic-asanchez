...
if(isset($data['form'])&&($data['form']==md5(fmed))){
	...
	if($acc==md5(UPDm)){
		...
		lab=%s, generico=%s, comercial=%s, presentacion=%s, cantidad=%s, descripcion=%s, idA=%s, estado=%s WHERE md5(id_form)=%s 
		LIMIT 1",
						SSQL($data['lab'], "text"),
						SSQL($data['generico'], "text"),
						SSQL($data['comercial'], "text"),
...
if(isset($acc)&&($acc==md5(DELi))){
	...
	if($TMC>0){
		...
		$qDEL=sprintf('DELETE FROM db_indicaciones WHERE md5(id)=%s LIMIT 1',
					  SSQL($ids,'text'));
		$LOG.=$qDEL.'<br>';
		if(@mysql_query($qDEL)){
			$vP=TRUE;
			$LOG.="<p>Indicacion Eliminado</p>";
		}else $LOG.='<p>No se pudo Eliminar</p>'.mysql_error();
	}
	$url.='?id='.$id;
}
